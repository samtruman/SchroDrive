 ...
 */
export function encodeWebDavPath(value: string): string {
  return value.split("/").map((segment) => encodeURIComponent(segment)).join("/");
}

/** Remove a provider file-tree root when it is repeated in a file path. */
export function webDavRelativeFilePath(torrentName: string, fileName: string): string {
...
function buildNestedFileResponses(
  baseHref: string,
  torrentName: string,
  files: VirtualFile[],
  lastModified: string,
  ...
  for (const file of files) {
    ...
    if (parts.length > 1) {
      if (!childCollections.has(childName)) {
        ...
        responses.push(buildCollectionResponse(
          `${baseHref}/${encodeWebDavPath(collectionPath)}/`,
          childName,
        ));
      }
      continue;
    ...
    responses.push(buildFileResponse(
      `${baseHref}/${encodeWebDavPath(filePath)}`,
      childName,
      file.size,
      lastModified,
    ));
...
class BridgeCache {
  ...
  private staleTorrentList: VirtualDirectory[] | null = null;
  ...
  private staleTorrentInfo: Map<string, VirtualFile[]> = new Map();
   ...
   */
  getStaleTorrentList(): VirtualDirectory[] | null {
   ...
   */
  getStaleTorrentInfo(torrentId: string): VirtualFile[] | null {
...
async function resolveRDDownloadUrl(torrentId: string, linkIndex: number): Promise<string | null> {
  ...
  if (rateLimiter.isRateLimited(providerName) && !isRotated) {
...
export class WebDAVBridge {
  ...
  private async handlePropfind(req: Request, res: Response): Promise<void> {
    ...
    if (isMediaView(firstSegment)) {
      ...
      if (!fileName) {
        ...
        if (depth !== "0") {
          responses.push(...buildNestedFileResponses(
            `/${encodeURIComponent(view)}/${encodeURIComponent(dir.name)}`,
            dir.name,
            files,
            lastModified,
          ));
      ...
      if (!file && files.some((f) => {
        ...
        const responses = [buildCollectionResponse(
          `${baseHref}/${encodeWebDavPath(relativePath)}/`,
   ...
   */
  private flagTorrentAsDead(dir: VirtualDirectory, reason: string): void {
    ...
    logError(
      this.provider,
      `Torrent ${dir.name} (${dir.id}) flagged as dead: ${reason} (failures: ${currentFailures})`
    );
        ...
        async () => {
          ...
          if (this.provider === "realdebrid") {
            if (typeof file.linkIndex !== "number") {
              throw new UnplayableTorrentError(`No link index for file ${file.name} in torrent ${dir.name}`);
            }
